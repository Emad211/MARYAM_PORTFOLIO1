...
export async function getMySubmissions(): Promise<SubmissionWithTask[]> {
    ...
    const out: SubmissionWithTask[] = [];
    for (const raw of (data ?? []) as SubmissionRow[]) {
        const embedded = raw.tasks;
        const taskRow = Array.isArray(embedded) ? embedded[0] : embedded;
        if (!taskRow) continue;
        out.push(mapSubmissionRow(raw, mapTaskRow(taskRow)));
    }
    return out;
}
